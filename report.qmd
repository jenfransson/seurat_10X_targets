...
For this analysis, the R package `targets` is used to manage the workflow. This enables caching of intermediate files without manual management, thus reducing risk of mismatched intermediate files after analysis parameters have been modified.

However, this means that the code in this report does not represent the full code necessary to generate the figures. Instead, the code used to produce the R objects used here can be found primarily in the files _targets.R and R/functions.R. To see which functions and parameters have generated each object, the following code can be run within the repository directory to generate a workflow chart:
...
Documentation about the package can be found [here](https://books.ropensci.org/targets/).

The workflow is checked and rerun if necessary using the command `tar_make()` or `tar_make_future()` (the latter enables parallelization).
...
# Enables changing targets directory (Use tar_config_set to set up new project), e.g.
...
In addition, genes were removed from the data sets unless they were expressed in > `r tar_read(qc_minCells)` cells.
...
if(tar_read(qc_runDF)){
  DFmessage = 'In droplet-based sequencing, a number of droplets containing more than one cell is expected. In order to predict which "cells" in the dataset are actually composed of more than one cell, the package [DoubletFinder](https://github.com/chris-mcginnis-ucsf/DoubletFinder) is used. Doublet cells are then removed from the dataset. To inform how many doublets could be expected, the column expectedCells in the metadata was used."
...
tar_load(dimred_pca_parameters)

if(length(dimred_pca_parameters)>0){
  pca_par_used = TRUE
  if(!is.null(dimred_pca_parameters$features)){
    pca_feature_text = "a custom list of genes."
  }
}
if(!exists("pca_feature_text")){
  pca_feature_text = paste0("the top ",tar_read(dimred_nHVG)," most variable features.")
}

```


Dimensional reduction was performed using PCA based on `r pca_feature_text` They are visualized below (in red), showing the mean expression and standardized variance for each gene. The top 20 genes are labeled.
...
LabelPoints(plot = tar_read(dimred_varfeat_plot), points = top20, repel = TRUE)
```
...
The distribution of QC variables and cell cycle scores are shown across clusters using violin plots.
...
Differential expression analysis was performed between clusters by comparing each cluster to all other clusters ("1-vs-all").

The top 10 differentially expressed genes (DEGs) per cluster (based on p-value) are listed below. The bars show the log2(fold-change) of the DEG.
...
Expression of the top 3 DEGs per cluster (based on p-value) are shown in across clusters in a dot plot and violin plots.
...
obj_clus@active.assay = "RNA"

```


```{r, fig.height=5, fig.width=5}
DotPlot(obj_clus, features = rev(unique(top3)), group.by = "Cluster") + coord_flip() + 
...
Below are any warnings produced during the targets workflow.
